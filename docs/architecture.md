...
bulkhead/
  packages/
    ...
    server/     @bulkhead/server    HTTP REST + MCP server (depends on core)
...
```
@bulkhead/core        (no internal deps — standalone library)
    ^         ^
    |         |
bulkhead   @bulkhead/server
(vscode)   (fastify + MCP SDK)
```

`@bulkhead/core` is the shared detection engine. Both the VS Code extension and the
...
The cascade is reachable through five different entry points, all of which converge on the same `@bulkhead/core` engine:
